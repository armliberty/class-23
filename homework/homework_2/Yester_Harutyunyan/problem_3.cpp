...
int main() {
    ...
    if (!Index.empty()) {
        for (StateId i = 0; i < Index.size(); i++) {
            print(Index[i] + 1, "First symbol position");
            print(Index[i] + pattern.size(), "Last symbol position");
        }
    } 
    else {
...
StateId computeNextState(StateId currStateId, char c, const std::string& pattern) {
    const auto finalState = pattern.size();
    if (std::tolower(c) == pattern[currStateId] || std::toupper(c) == pattern[currStateId] || '.' == pattern[currStateId] && currStateId != finalState) {
...
std::vector<int> fsmFindFirstMatch(const TrMatrix& trMx, StateId finalState, const std::string& text) {
    ...
    for (size_t i = 0; i < text.size(); ++i) {
        ...
        if (currState == finalState) {
            allOccurrences.push_back(i - finalState);
        }
    }
