...
template <typename T>
T input(std::string msg,int &ind) {
    if (!msg.empty()) {
        std::cout << msg << ": ";
    }
    T value;
...
template <>
std::string input(std::string msg,int & ind) {
    if (!msg.empty()) {
        std::cout << msg << ": ";
    }
    std::string value;
...
TrMatrix computeTransitionMatrix(const std::string& pattern, int & ind);
std::optional<size_t> fsmFindFirstMatch(const TrMatrix& trMx, StateId finalState, const std::string& text);

int main() {
    int ind;
    const auto pattern = input<std::string>("Input search pattern",ind);
    const auto text = input<std::string>("Input text to search",ind);
    const auto trFunc = computeTransitionMatrix(pattern,ind);
...
StateId computeNextState(StateId currStateId, char c, const std::string& pattern, int & ind) {
    const auto finalState = pattern.size();
...
TrMatrix computeTransitionMatrix(const std::string& pattern, int & ind) {
    ...
    for (StateId stateId = 0; stateId < stateCount; ++stateId) {
        for (char c = 0; c < alfabetSize; ++c) {
            trFunc[stateId][c] = computeNextState(stateId, c, pattern,ind);
        }
    }
