...
template <typename T>
T input(std::string msg, int &pointIndex)
{
	if (!msg.empty())
	{
		std::cout << msg << ": ";
...
template <>
std::string input(std::string msg, int &pointIndex)
{
	if (!msg.empty())
	{
		std::cout << msg << ": ";
...
TrMatrix computeTransitionMatrix(const std::string &pattern, int &pointIndex);
std::vector<int> fsmFindFirstMatch(const TrMatrix &trMx, StateId finalState, const std::string &text);

int main()
{
	int pointIndex;
	const auto pattern = input<std::string>("Input search pattern", pointIndex);
	const auto text = input<std::string>("Input text to search", pointIndex);
	const auto trFunc = computeTransitionMatrix(pattern, pointIndex);
...
StateId computeNextState(StateId currStateId, char c, const std::string &pattern, int &pointIndex)
{
	const auto finalState = pattern.size();
	if (currStateId == pointIndex)
	{
...
TrMatrix computeTransitionMatrix(const std::string &pattern, int &pointIndex)
{
	...
	for (StateId stateId = 0; stateId < stateCount; ++stateId)
	{
		...
		{
			trFunc[stateId][c] = computeNextState(stateId, c, pattern, pointIndex);
		}
	}
