...
int main() {
    ...
    const auto vect = fsmFindFirstMatch(trFunc, finalState, text);
    int id_occ=1;
    if(vect.size()==0) {
        print("Pattern not found");
            ...
            std::cout<<"id_occ:"<<id_occ<<std::endl;
            print(vect[i]-(pattern.size()-1), "First symbol position");
            print(vect[i],"Last symbol position");
            ++id_occ;
        }
    }
}
