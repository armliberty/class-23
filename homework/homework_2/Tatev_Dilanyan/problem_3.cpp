...
template <typename T>
T input(std::string msg, int& index) {
    if (!msg.empty()) {
        std::cout << msg << ": ";
    }
    T value;
...
template <>
std::string input(std::string msg, int& index) {
    if (!msg.empty()) {
        std::cout << msg << ": ";
    }
    std::string value;
...
TrMatrix computeTransitionMatrix(const std::string& pattern, const int index);
std::optional<size_t> fsmFindFirstMatch(const TrMatrix& trMx, StateId finalState, const std::string& text);

int main() {
    int index;
    const auto pattern = input<std::string>("Input search pattern", index);
    const auto text = input<std::string>("Input text to search", index);
    const auto trFunc = computeTransitionMatrix(pattern, index);
    const auto finalState = pattern.size();
    const auto optIndex = fsmFindFirstMatch(trFunc, finalState, text);
    if (optIndex.has_value()) {
        print((optIndex.value() + 1)- pattern.size(), "First symbol position");
...
StateId computeNextState(StateId currStateId, char c, const std::string& pattern, const int index) {
    const auto finalState = pattern.size();
    if (currStateId == index) {
...
TrMatrix computeTransitionMatrix(const std::string& pattern, const int index) {
    ...
    for (StateId stateId = 0; stateId < stateCount; ++stateId) {
        for (char c = 0; c < alfabetSize; ++c) {
            trFunc[stateId][c] = computeNextState(stateId, c, pattern, index);
        }
    }
