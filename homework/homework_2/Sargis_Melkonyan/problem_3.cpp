/*
3. Modify the FSM construction in stirng matching algorithm to 
recognize '.' wildcard matching any character.
*/
...
StateIds fsmFindMatch(const TrMatrix& trMx, StateId finalState, const std::string& text);
std::string toLower(std::string inPattern);
...
int main() {
    ...
    const auto optIndexes = fsmFindMatch(trFunc, finalState, text);
    if (optIndexes.empty())
         print("Pattern not found");
    else
        for(int i = 0; i < optIndexes.size(); ++i){
                print(optIndexes[i] - pattern.size() + 1, "First symbol position");
                print(optIndexes[i], "Last symbol position");
            } 
}
